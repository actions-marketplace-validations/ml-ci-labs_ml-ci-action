...
_Real GitHub PR comment generated by ML-CI v0.3.0._
...
ML-CI now supports an optional repo-root policy file at `.ml-ci.yml`. If present, the action auto-discovers it and uses it as the default gating policy for every workflow in the repository.
...
- The action looks only for `.ml-ci.yml` at the repo root in Sprint 2.
- Supported schema keys are `version` and `policy`.
- `policy.metrics.<name>.severity` supports `fail` and `warn`.
- `policy.metrics.<name>.direction` supports `higher` and `lower`.
...
The screenshot above is a real GitHub PR comment produced by ML-CI v0.3.0. Each run creates or updates one idempotent comment with the following sections:
...
---
*Generated by [ML-CI](https://github.com/ml-ci-labs/ml-ci-action) v0.3.0*
```
...
CSV and Parquet are supported in v0.3.0.
...
- No dashboard or billing flow in v0.3.0
...
| `ml-ci-action` | One-step GitHub-native validation and reporting | No hosted history or compliance layer in v0.3.0 |
