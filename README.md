...
![Example ML-CI PR comment from a real GitHub pull request using v0.2.1](./docs/assets/pr-comment-screenshot.png)

_Real GitHub PR comment generated by ML-CI v0.2.1._
  ...
  validate:
    ...
    steps:
      ...
      - name: Validate model changes
        uses: ml-ci-labs/ml-ci-action@v0.2.1
        with:
...
The screenshot above is a real GitHub PR comment produced by ML-CI v0.2.1. Each run creates or updates one idempotent comment with the following sections:
...
---
*Generated by [ML-CI](https://github.com/ml-ci-labs/ml-ci-action) v0.2.1*
```
...
CSV and Parquet are supported in v0.2.1.
...
- No dashboard or billing flow in v0.2.1
...
| `ml-ci-action` | One-step GitHub-native validation and reporting | No hosted history or compliance layer in v0.2.1 |
